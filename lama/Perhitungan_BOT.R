...
dt = read.csv(file = "gempa.txt", header = F, sep = "|") 
dt = as.matrix(dt[1,])
...
mencari_titik_terdekat = function(xy){
  ...
  # i_dekat = which(jarak == min(jarak))[1]
  # hasil = as.matrix(SULSEL_POINT[which(jarak == min(jarak)),])
  # gempa = cbind(lng = as.numeric(as.matrix(xy$lng)), lat = as.numeric(as.matrix(xy$lat)))
  # kecamatan = cbind(lng = SULSEL_POINT$x[i_dekat], lat = SULSEL_POINT$y[i_dekat] )
  # arah = finalBearing(kecamatan,gempa)
  # if(arah < 0){
  #   arah = 360 + arah
  # }
  # directiong = dari_arah_mana(arah) 
  # 
  # dimana = paste(as.matrix(hasil[,3:5]), collapse = ", ") 
  # dimana = as.character(as.matrix(hasil[,3:5])) 
  # jrk = round(jarak[which(jarak == min(jarak))], digits = 2) # <<
  # jrk = as.integer(jrk)
  # dimana = data.frame( kecamatan,
  #                      # X_kec = kecamatan$lng, #1
  #                      # Y_kec = kecamatan$lat, #1
  #                      jarak = jrk , #1
  #                      arah = directiong, #1
  #                      Kec = str_to_title(dimana[1]), Kab = str_to_title(dimana[2]), Prov = str_to_title(dimana[3])) #3
  # 
  # dimana = paste0(round(jarak[which(jarak == min(jarak))], digits = 2), " Km ",directiong," dari Kec. ", str_to_title(dimana))
...
if(darat == 1){
   # messagex = paste0(messagenya, " (","Terjadi di darat pada ",hhhh$jarak," Km ",
  # messagex = paste0(messagenya, " ("," ",hhhh$jarak," Km ",
  # as.character(hhhh$arah),
  # " Kec. ",paste(as.matrix(hhhh[,5:7]), collapse = ", "),
  # "), Kedlmn: ",dep, "::BMKG-PGRIV"   )
  
  messagex = paste0(messagenya, " \n    terjadi di darat jarak  :\n      "," ",paste(hhhh, collapse = ";\n       "),
                    # " Kec. ",paste(as.matrix(hhhh[,5:7]), collapse = ", "),
                    " \n Kedlmn: ",dep, "::BMKG-PGRIV"   )
}else{
  messagex = paste0(messagenya, " \n    terjadi di laut jarak  :\n      "," ",paste(hhhh, collapse = ";\n       "),
                    # " Kec. ",paste(as.matrix(hhhh[,5:7]), collapse = ", "),
                    " \n Kedlmn: ",dep, "::BMKG-PGRIV"   )
}

write.table(file = "message_warn.txt", messagex, col.names = F, row.names = F)
