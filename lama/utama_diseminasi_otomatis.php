...
$conn = new PDO("mysql:host=$servername;dbname=$database", $username, $password);
	$query = $conn->prepare( "SELECT * FROM data_gempa WHERE waktu_lokal is NOT NULL ORDER BY tanggal DESC, waktu DESC LIMIT 20" );
	$query->execute();
?>
...
<body>
	<table id="tabelutama">
		...
		while($row=$query->fetch(PDO::FETCH_ASSOC)) {
			...
			echo "<td>".'<button id="tombol_kirim_data" class="tombol" onclick="kirim_data('.$x.','."'".$row['waktu_lokal']."'".')">Kirim</button>'."</td>";
			//echo "<td>".'<button id="tombol_cek_detail_gambar" class="tombol" onclick="simpan_gambar('.$x.')">Cek</button>'."</td>";
			echo "</tr>";
			$x++;
		}
...
<script>
	...
	function kirim_data(urutan,waktulokal) {
		...
		if(longtitude_benar[1]=="W"){
			longtitude_benar[0] = "-"+longtitude_benar[0];
              ...
              window.open('/proses_kirimdata_all.php?waktu_lokal='+waktulokal+'&lat='+latitude_benar[0]+'&lon='+longtitude_benar[0],'_self');	
	}
	function simpan_gambar(urutan) {
              	var latitude_benar = latitude[urutan].split(" ");
		var longtitude_benar = longtitude[urutan].split(" ");
		if(latitude_benar[1]=="S"){
			latitude_benar[0] = "-"+latitude_benar[0];
		...
		var alamat_peta = document.getElementById("peta").src;
		window.open('/proses_kirimgambar.php?alamat_peta='+alamat_peta,'_self');
	}
